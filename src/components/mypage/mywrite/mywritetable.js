        ...
        return (
            <div className="body_sub_form">
                <div /*key={}*/ className='likeit_content_one'>
                    ...
                    <div onClick={()=>show_mywrite(user.number)} className='mywrite_title_content'>{user.title}</div>
                    ...
                    <div className='mywrite_cancel' style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }} >
                        <button className='likeit_cancel_btn' onClick={(e) => Func_this_delete_content(e, user.number)}>취소</button>
                    </div>
                </div>
            </div>
        )
